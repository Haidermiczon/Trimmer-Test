...
public enum MaskingMode {
    case dimUnselectedArea  // default (left + right masked)
    case dimSelectedArea    // reverse (center masked)
}
...
@IBDesignable public class TrimmerView: AVAssetTimeSelector {
    ...
    /// Masking properties
    public var maskingMode: MaskingMode = .dimSelectedArea {
        didSet {
            updateMaskingMode()
        }
    }
    ...
    @IBInspectable public var maskColor: UIColor = UIColor.white {
        didSet {
            [leftMaskView, rightMaskView, centerMaskView].forEach {
                $0.backgroundColor = maskColor
            }
        }
    }
    ...
    private let centerMaskView = UIView() // For selected area masking
    ...
    private let handleWidth: CGFloat = 2
    ...
    private func setupMaskView() {
        // Shared config
        [leftMaskView, rightMaskView, centerMaskView].forEach { view in
            view.isUserInteractionEnabled = false
            view.backgroundColor = maskColor
            view.alpha = 0.7
            view.translatesAutoresizingMaskIntoConstraints = false
            insertSubview(view, belowSubview: trimView)
        }
        ...
        // CENTER MASK
        NSLayoutConstraint.activate([
            centerMaskView.leftAnchor.constraint(equalTo: leftHandleView.centerXAnchor),
            centerMaskView.rightAnchor.constraint(equalTo: rightHandleView.centerXAnchor),
            centerMaskView.topAnchor.constraint(equalTo: topAnchor),
            centerMaskView.bottomAnchor.constraint(equalTo: bottomAnchor)
        ])
        
        updateMaskingMode()
    }
    
    private func updateMaskingMode() {
        switch maskingMode {
        case .dimUnselectedArea:
            leftMaskView.isHidden = false
            rightMaskView.isHidden = false
            centerMaskView.isHidden = true
        case .dimSelectedArea:
            leftMaskView.isHidden = true
            rightMaskView.isHidden = true
            centerMaskView.isHidden = false
        }
    }
    ...
    private func updateSelectedTime(stoppedMoving: Bool) {
        ...
        print("Selected Time: \(playerTime)")
    }
